...
public class ToDoController {
    ...
    public ResponseEntity<Iterable<ToDo>> getToDos() {
        return ResponseEntity.ok(toDoRepositoryImpl.findAll());
    }

    @GetMapping("/todo/{id}")
    public ResponseEntity<ToDo> getToDoById(@PathVariable String id) {
        Optional<ToDo> toDo = toDoRepositoryImpl.findById(id);
        if (toDo.isPresent())
    ...
    public ResponseEntity<ToDo> setCompleted(@PathVariable String id) {
        ...
        toDoRepositoryImpl.save(result);
    ...
    public ResponseEntity<?> createToDo(@Valid @RequestBody ToDo toDo,
        ...
        ToDo result = (ToDo) toDoRepositoryImpl.save(toDo);
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().
    ...
    public ResponseEntity<ToDo> deleteToDo(@PathVariable String id) {
        toDoRepositoryImpl.delete(ToDoBuilder.create().withId(id).build());
